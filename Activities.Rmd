...
full_act%>% group_by(parkCode) %>% count()%>% ggplot(aes(x=parkCode,y=n))+geom_point()+theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
```
...
As part of promotional material the NPS also has a list of things to do. These are specific programs or trails that a person might try at the park. They are catogorized under the different activities listed above. We have a little more information about these activites. Below is a breakdown of some of the things that might effect participation 
```{r}
