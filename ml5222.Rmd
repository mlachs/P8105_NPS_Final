...
NPS_Full %>% drop_na %>% select(entranceFees_cost,entranceFees_title) %>% distinct() %>% ggplot(aes(x=entranceFees_title,y=entranceFees_cost))+geom_boxplot()+theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
  ...
  filter(entranceFees_title=='Entrance - Per Person') %>% distinct()
```

```{r}
library(leaflet)

NPS_Full %>% mutate(latitude=as.numeric(latitude),longitude=as.numeric(longitude)) %>% select(fullName,latitude,longitude) %>% distinct() %>% leaflet() %>% addTiles() %>% addMarkers(~longitude,~latitude)


NPS_Full %>% mutate(latitude=as.numeric(latitude),longitude=as.numeric(longitude)) %>%
  filter(
         entranceFees_cost %in% 0:30)


clean_things %>% mutate(latitude=as.numeric(latitude),longitude=as.numeric(longitude)) %>%
  select(fullName,latitude,longitude) %>% distinct() 
```
